...
int wiringPiI2CReadReg8 (int fd, uint8_t reg)
{
  union i2c_smbus_data data;
...
int wiringPiI2CWriteBulk(int fd, int reg, uint8_t *value, int size)
{
	union i2c_smbus_data data;

	data.block[0] = size;
	for (int i = 1; i <= size; i++)
		data.block[i] = value[i - 1];
